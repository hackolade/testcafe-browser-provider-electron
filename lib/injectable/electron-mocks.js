...
var ELECTRON_VERSION = process.versions.electron && Number(process.versions.electron.split('.')[0]);

var ELECTRON_VERSION_WITH_ASYNC_LOAD_URL = 5;
...
function getWebContents() {
    // NOTE: < Electron 6
    if (process.atomBinding) return process.atomBinding('web_contents').WebContents;
    // NOTE: >= Electron 6
    else if (process.electronBinding) return process.electronBinding('web_contents').WebContents;
...
module.exports = function install(config, testPageUrl) {
    ...
    var ipcConnectionPromise = ipc.connect();
    ...
    function loadUrl(webContext, url, options) {
        ...
        if (isFileProtocol(url)) testUrl = (0, _resolveFileUrl2.default)(config.appEntryPoint, testUrl);
        ...
        if (testUrl.toLowerCase() === config.mainWindowUrl.toLowerCase()) {
            ...
            if (config.openDevTools) webContext.openDevTools();
        }
    ...
    function loadURLWrapper(url, options) {
        ...
        if (ELECTRON_VERSION >= ELECTRON_VERSION_WITH_ASYNC_LOAD_URL) return ipcConnectionPromise.then(function () {
            return loadUrl(_this, url, options);
        });
    ...
    if (WebContents.prototype._init) {
        var savedWebContentsInit = WebContents.prototype._init;
    ...
    } else WebContents.prototype.loadURL = loadURLWrapper;
